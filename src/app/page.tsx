  ...
  return (
    <main className="min-h-screen bg-white">
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        <p className="text-gray-600 mt-1">Instant EMI, interest & payoff visualization. Shareable URL.</p>

        <div className="grid md:grid-cols-2 gap-8 mt-8">
          <section className="space-y-6">
            <SliderRow label="Loan amount" min={50000} max={5000000} step={10000}
              value={principal} onChange={(v) => setParam("amount", v)} prefix="₹ " />
            <SliderRow label="Rate of interest (p.a.)" min={5} max={24} step={0.1}
              value={rate} onChange={(v) => setParam("rate", v)} suffix=" %" />
            <SliderRow label="Loan tenure (months)" min={6} max={360} step={1}
              value={months} onChange={(v) => setParam("months", v)} />
